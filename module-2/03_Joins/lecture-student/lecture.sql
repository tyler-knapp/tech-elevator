...
SELECT payment.payment_id, payment.rental_id, customer.first_name, customer.last_name, rental.return_date, film.title, ARRAY_AGG(actor.first_name || ' ' || actor.last_name)
FROM payment 
JOIN customer ON payment.customer_id = customer.customer_id
JOIN rental ON payment.rental_id = rental.rental_id
JOIN inventory ON rental.inventory_id = inventory.inventory_id
...
WHERE payment_id =16666
GROUP BY payment.payment_id, payment.rental_id, customer.first_name, customer.last_name, rental.return_date, film.title;
